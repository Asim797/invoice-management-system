...
const addCart = (item) => {
    ...
    form.value.invoice_items.push(itemObj);
    handleModal();
}
...
const onSave = async () => {
    if (form.value.invoice_items.length >= 1){
        ...
        let response = await axios.post('/api/update-invoice/'+form.value.id, formData)
        if (response.data.status == 'success'){
            form.value.invoice_items = [];
            router.push('/invoices/show/'+form.value.id);
        }else{
            ...
            <div class="card__content">
                <div class="card__content--header">
                    <div>
                        ...
                        <select name="" id="" v-model="form.customer_id" class="input">
                ...
                <div class="table">
                    ...
                    <div class="table--items2" v-for="(itemCart, i) in form.invoice_items" :key="itemCart.id">
                        ...
                        <p v-else></p>
                        <p style="color: red; font-size: 24px;cursor: pointer;" @click="removeItem(i, itemCart?.id)">
                            &times;
                        </p>
                    </div>
